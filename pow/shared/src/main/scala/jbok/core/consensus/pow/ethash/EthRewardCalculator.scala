...
/**
  ...
  */
class EthRewardCalculator(config: MonetaryPolicyConfig = MonetaryPolicyConfig()) {
  ...
  /**
    ...
    */
  private[jbok] def calcMinerBaseReward(era: Int): BigInt = {
  ...
  /**
    ...
    */
  private[jbok] def calcMinerRewardPerOmmer(era: Int): BigInt =
    calcMinerBaseReward(era) * ommerInclusionRewardNumer / ommerInclusionRewardDenom

  /** era number counting from 0 */
  private[jbok] def eraNumber(blockNumber: BigInt): Int =
