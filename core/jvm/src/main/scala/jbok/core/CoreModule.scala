...
class CoreModule[F[_]: TagK](config: FullConfig)(implicit F: ConcurrentEffect[F], cs: ContextShift[F], T: Timer[F]) extends ModuleDef {
  ...
  def makeQueues(): Unit ={
    ...
    make[Queue[F, Peer[F], Status]].fromEffect(MemoryQueue[F, Peer[F], Status])
    make[Queue[F, PeerSelector[F], Status]].fromEffect(MemoryQueue[F, PeerSelector[F], Status])
  }
  makeQueues()
  ...
  def makeCP(): Unit ={
    ...
    make[Consumer[F, Peer[F], Status]].from((queue: Queue[F, Peer[F], Status]) => queue)
    make[Producer[F, Peer[F], Status]].from((queue: Queue[F, Peer[F], Status]) => queue)

    make[Consumer[F, PeerSelector[F], Status]].from((queue: Queue[F, PeerSelector[F], Status]) => queue)
    make[Producer[F, PeerSelector[F], Status]].from((queue: Queue[F, PeerSelector[F], Status]) => queue)
  }
