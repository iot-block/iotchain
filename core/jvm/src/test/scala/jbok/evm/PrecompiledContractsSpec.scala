...
class PrecompiledContractsSpec extends JbokSpec { 
  ...
  "ECDSARECOVER" in {
    ...
    forAll(bytesGen) { bytes =>
      ...
      val validSig = SecP256k1.sign[IO](hash, keyPair).unsafeRunSync()
      val recoveredPub = SecP256k1.recoverPublic(validSig, hash).get
      val recoveredAddress = recoveredPub.uncompressed.kec256.slice(12, 32).padLeft(32)
      val inputData = hash ++ UInt256(validSig.v.get).bytes ++ UInt256(validSig.r).bytes ++ UInt256(validSig.s).bytes
