...
object StatefulGen {
  ...
  def transactions(min: Int = 0, max: Int = 32, history: History[IO] = locator.unsafeRunSync().get[History[IO]])(implicit config: FullConfig): Gen[List[SignedTransaction]] =
    for {
  ...
  def blocks(min: Int = 1, max: Int = 1, parent: Option[Block] = None, stxs: Option[List[SignedTransaction]] = None)(implicit config: FullConfig): Gen[List[Block]] = {
  ...
  def block(parent: Option[Block] = None, stxs: Option[List[SignedTransaction]] = None): Gen[Block] = {
    val objects = locator.unsafeRunSync()
    val miner   = objects.get[BlockMiner[IO]]
    miner.mine(parent, stxs).unsafeRunSync().block
  }
}
