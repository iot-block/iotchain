...
class TransactionValidatorSpec extends JbokSpec {
  ...
  "TxValidator" should {
    "report as valid a tx from before homestead" in {
    ...
    "report as valid a tx from after homestead" in {
      transactionValidator
    ...
    "report a tx with invalid s as having invalid signature before homestead" in {
      forAll(bigIntGen) { s =>
