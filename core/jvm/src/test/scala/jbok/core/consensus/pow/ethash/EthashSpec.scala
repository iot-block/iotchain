...
class EthashSpec extends JbokSpec {
  ...
  "Ethash" should {
    ...
    "compute proof of work using cache" ignore {
      ...
      val mixHash = Array(0x1f, 0xff, 0x04, 0xce, 0xc9, 0x41, 0x73, 0xfd, 0x59, 0x1e, 0x3d, 0x89, 0x60, 0xce, 0x6b,
        0xdf, 0x8b, 0x19, 0x71, 0x04, 0x8c, 0x71, 0xff, 0x93, 0x7b, 0xb2, 0xd3, 0x2a, 0x64, 0x31, 0xab,
        0x6d).map(_.toByte)
      val boundary = Array(0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x3e, 0x9b, 0x6c, 0x69, 0xbc, 0x2c, 0xe2, 0xa2, 0x4a,
        0x8e, 0x95, 0x69, 0xef, 0xc7, 0xd7, 0x1b, 0x33, 0x35, 0xdf, 0x36, 0x8c, 0x9a, 0xe9, 0x7e, 0x53,
        0x84).map(_.toByte)
      ...
      proofOfWork.difficultyBoundary shouldBe ByteVector(boundary)
      ...
      forAll(table) { (blockNumber, hashWithoutNonce, nonce, mixHash) =>
        ...
        val proofOfWork = hashimotoLight(
          ByteVector.fromValidHex(hashWithoutNonce).toArray,
          ByteVector.fromValidHex(nonce).toArray,
          dagSize(epoch(blockNumber)),
          cache
        )
        proofOfWork.mixHash shouldBe ByteVector.fromValidHex(mixHash)
      }
    }
  }
}
