  ...
  def check(newConsensus: () => ConsensusFixture): Unit =
    "FullSync" should {
      "sync all nodes to a single highest state" in {
        ...
        val miner1 = new BlockMinerFixture(fixture1, NetAddress("localhost", 20000))
        miner1.peerManager.listen.unsafeRunSync()

        val fixture2 = newConsensus()
        val miner2 = new BlockMinerFixture(fixture2, NetAddress("localhost", 20001))
        ...
        miner2.peerManager.connect(miner1.peerManager.localAddress.unsafeRunSync()).unsafeRunSync()
        Thread.sleep(2000)
