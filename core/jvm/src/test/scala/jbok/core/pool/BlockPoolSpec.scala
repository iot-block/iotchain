...
class BlockPoolSpec extends CoreSpec {
  "BlockPool" should {
    ...
    "remove a branch from a leaf up to the first shared ancestor" in {
      ...
      val block2a = miner.mine1(block1.some, randomTx).unsafeRunSync().right.get.block
      val block2b = miner.mine1(block1.some, randomTx).unsafeRunSync().right.get.block
      val block2c = miner.mine1(block1.some).unsafeRunSync().right.get.block
      val block3  = miner.mine1(block2a.some).unsafeRunSync().right.get.block
    ...
    "remove a whole subtree down from an ancestor to all its leaves" in {
      ...
      val block1a = miner.mine1(genesis.some, txs.take(1).some).unsafeRunSync().right.get.block
      val block1b = miner.mine1(genesis.some).unsafeRunSync().right.get.block
      val block2a = miner.mine1(block1a.some, txs.takeRight(1).some).unsafeRunSync().right.get.block
      val block2b = miner.mine1(block1a.some).unsafeRunSync().right.get.block
      val block3  = miner.mine1(block2a.some).unsafeRunSync().right.get.block
