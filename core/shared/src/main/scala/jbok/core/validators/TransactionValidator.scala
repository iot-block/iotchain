...
class TransactionValidator[F[_]](blockChainConfig: BlockChainConfig)(implicit F: Sync[F]) {
  ...
  /**
    ...
    */
  private def checkSyntacticValidity(
      stx: SignedTransaction): EitherT[F, SignedTransactionInvalid, SignedTransaction] = {
    import stx._
    ...
    if (nonce > maxNonceValue)
      EitherT.leftT(TransactionSyntaxInvalid(s"Invalid nonce: $nonce > $maxNonceValue"))
    else if (gasLimit > maxGasValue)
      EitherT.leftT(TransactionSyntaxInvalid(s"Invalid gasLimit: $gasLimit > $maxGasValue"))
    else if (gasPrice > maxGasValue)
      EitherT.leftT(TransactionSyntaxInvalid(s"Invalid gasPrice: $gasPrice > $maxGasValue"))
    else if (value > maxValue)
      EitherT.leftT(TransactionSyntaxInvalid(s"Invalid value: $value > $maxValue"))
    else if (r > maxR)
      EitherT.leftT(TransactionSyntaxInvalid(s"Invalid signatureRandom: $r > $maxR"))
    else if (s > maxS)
      EitherT.leftT(TransactionSyntaxInvalid(s"Invalid signature: $s > $maxS"))
    else
      EitherT.rightT(stx)
  }
  ...
  /**
    ...
    */
  private def validateSignatureFormat(stx: SignedTransaction,
                                      blockNumber: BigInt): EitherT[F, SignedTransactionInvalid, SignedTransaction] = {
    import stx._
...
//    val validSigningSchema = if (beforeEIP155) !stx.isChainSpecific else true
    ...
    if (validR && validS && validSigningSchema) EitherT.rightT(stx)
    else EitherT.leftT(TransactionSignatureInvalid)
  }
  ...
  /**
    ...
    */
  private def validateNonce(nonce: BigInt, senderNonce: UInt256): EitherT[F, SignedTransactionInvalid, BigInt] =
    if (senderNonce == UInt256(nonce)) EitherT.rightT(nonce)
    else EitherT.leftT(TransactionNonceInvalid(UInt256(nonce), senderNonce))
  ...
  private def validateAccountHasEnoughGasToPayUpfrontCost(
      ...
      upfrontCost: UInt256): EitherT[F, SignedTransactionInvalid, UInt256] =
    if (senderBalance >= upfrontCost) EitherT.rightT(senderBalance)
    else EitherT.leftT(TransactionSenderCantPayUpfrontCostInvalid(upfrontCost, senderBalance))
  ...
  private def validateGasLimitEnoughForIntrinsicGas(
      ...
      blockHeaderNumber: BigInt): EitherT[F, SignedTransactionInvalid, SignedTransaction] = {
    ...
    if (stx.gasLimit >= txIntrinsicGas) EitherT.rightT(stx)
    else EitherT.leftT(TransactionNotEnoughGasForIntrinsicInvalid(stx.gasLimit, txIntrinsicGas))
  }
  ...
  private def validateBlockHasEnoughGasLimitForTx(gasLimit: BigInt,
                                                  accumGasUsed: BigInt,
                                                  blockGasLimit: BigInt): EitherT[F, SignedTransactionInvalid, BigInt] =
    if (gasLimit + accumGasUsed <= blockGasLimit) EitherT.rightT(gasLimit)
    else EitherT.leftT(TransactionGasLimitTooBigInvalid(gasLimit, accumGasUsed, blockGasLimit))
  ...
  def validate(
      ...
      accumGasUsed: BigInt
  ): EitherT[F, SignedTransactionInvalid, SignedTransaction] =
    for {
      ...
      _ <- validateBlockHasEnoughGasLimitForTx(stx.gasLimit, accumGasUsed, blockHeader.gasLimit)
    } yield stx
}
