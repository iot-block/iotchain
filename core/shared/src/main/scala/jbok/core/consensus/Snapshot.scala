...
object Snapshot {
  val namespace = ByteVector("snapshot".getBytes)
  ...
  def applyHeaders[F[_]](snapshot: Snapshot, headers: List[BlockHeader], chainId: BigInt)(
      implicit F: Sync[F]): F[Snapshot] =
    if (headers.isEmpty) {
      snapshot.pure[F]
    } else {
  ...
  private def applyHeader[F[_]](
      snap: Snapshot,
      header: BlockHeader,
      chainId: BigInt,
      signer: Address,
      authorize: Boolean
  )(implicit F: Sync[F]): F[Snapshot] = F.delay {
