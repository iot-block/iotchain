  ...
  private[jbok] lazy val nonEmptyCodec: RlpCodec[Node] = RlpCodec(
    rlist(ritem).narrow[Node](
      list =>
        list.size match {
          case 2 =>
            hp.decode(list.head).map {
              case (true, k) => LeafNode(k.toHex, list(1))
              case (false, k) => ExtensionNode(k.toHex, entryCodec.decode(list(1).bits).require.value)
            }
        ...
        case LeafNode(k, v) =>
          hp.encode(BitVector.fromHex(k).get, isLeaf = true) :: v :: Nil
        case ExtensionNode(k, v) =>
          hp.encode(BitVector.fromHex(k).get, isLeaf = false) :: entryCodec.encode(v).require.bytes :: Nil
