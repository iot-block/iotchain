...
case class AuthResponseMessage(ephemeralPublicKey: ByteVector, nonce: ByteVector, knownPeer: Boolean) {
  lazy val encoded: ByteVector = ByteVector(
    ephemeralPublicKey.drop(1).toArray ++ nonce.toArray ++ Array(if (knownPeer) 1.toByte else 0.toByte)
  )
}
object AuthResponseMessage {
  ...
  def decode(input: Array[Byte]): AuthResponseMessage =
    AuthResponseMessage(
      ephemeralPublicKey = ByteVector(0x04.toByte +: input.take(PublicKeyLength)),
      nonce = ByteVector(input.slice(PublicKeyLength, PublicKeyLength + NonceLength)),
      knownPeer = input(PublicKeyLength + NonceLength) == 1
    )
}
case class AuthResponseMessageV4(ephemeralPublicKey: ByteVector, nonce: ByteVector, version: Int)
