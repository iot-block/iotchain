...
object AuthInitiateEcdsaCodec {
  def encodeECDSA(sig: CryptoSignature): ByteVector = {
    ...
    val recoveryId: Byte = (sig.v.get - 27).toByte

    ByteVector(
      asUnsignedByteArray(sig.r.bigInteger).reverse.padTo(32, 0.toByte).reverse ++
        asUnsignedByteArray(sig.s.bigInteger).reverse.padTo(32, 0.toByte).reverse ++
        Array(recoveryId)
    )
  }
  ...
  def decodeECDSA(input: Array[Byte]): CryptoSignature = {
    ...
    CryptoSignature(BigInt(1, r), BigInt(1, s), Some(v.toByte))
  }
}
