...
class AuthConnectionSpec extends JbokSpec {
  ...
  val nodeId       = keyPair.public.bytes.toHex
  ...
  "AuthConnection" should {
    "build auth connection" in {
      val server =
        for {
          server <- tcp.server[IO, AuthConnection[IO]](9003)(socket => AuthConnection.accept(socket, handshaker))
          conn   <- server
      ...
      val client =
        for {
          conn <- tcp.client[IO, AuthConnection[IO]](9003)(socket =>
            AuthConnection.connect(socket, handshaker, nodeId))
