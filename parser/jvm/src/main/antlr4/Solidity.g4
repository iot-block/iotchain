...
contractPart
  : stateVariableDeclaration  # StateVariablePart
  | usingForDeclaration       # UsingPart
  | structDefinition          # StructPart
  | constructorDefinition     # ConstructorPart
  | modifierDefinition        # ModifierPart
  | functionDefinition        # FunctionPart
  | eventDefinition           # EventPart
  | enumDefinition            # EnumPart
  ;
...
variableModifiers
  ...
  | ConstantKeyword? (PublicKeyword | InternalKeyword | PrivateKeyword)
  |
  ;
...
parameterStatement
  : typeNameParam storageLocation? identifier? ;
...
typeNameParam
  : elementaryTypeName                        # PrimaryType
  | userDefinedTypeName                       # UserDefinedType
  | mapping                                   # MappingType
  | typeNameParam '[' constantExpression? ']' # ArrayType
  ;
...
mapping
  : 'mapping' '(' elementaryTypeName '=>' typeNameParam ')' ;
...
visibility
  : PublicKeyword | PrivateKeyword | ExternalKeyword | InternalKeyword ;
...
constantExpression
  : <assoc=right> op=('+' | '-') constantExpression             # UnaryOp
  | '(' constantExpression ')'                                  # Parens
  | <assoc=right> constantExpression op='**' constantExpression # BinaryOp
  | constantExpression op=('*' | '/' | '%') constantExpression  # BinaryOp
  | constantExpression op=('+' | '-') constantExpression        # BinaryOp
  | constantExpression op=('<<' | '>>') constantExpression      # BinaryOp
  | numberLiteralNoUnit                                         # Number
  ;
...
numberLiteral
  : numberLiteralNoUnit NumberUnit? ;
