...
//    "not remove a key if no more reference until pruning" in new Fixture {
...
//    }
//
//    "not remove a key that was inserted after deletion when pruning" in new Fixture {
//      val rc = RefCountKeyValueDB.forVersion(db, 1)
//      val inserted =
...
//    }
//
//    "not remove a key that it's still referenced when pruning" in new Fixture {
//      val rc = RefCountKeyValueDB.forVersion(db, 1)
//      val inserted =
//        (1 to 1).map(i => ByteVector.fromValidHex(s"dead$i") -> ByteVector.fromValidHex(s"beef${i}"))
//      inserted.foreach(t => rc.put(t._1, t._2).unsafeRunSync())
...
//    }
//
//    "not delete a key that's was referenced in later blocks when pruning" in new Fixture {
//      val rc = RefCountKeyValueDB.forVersion(db, 1)
//      val inserted =
...
//    }
//
//    "not throw an error when deleting a key that does not exist" in new Fixture {
//      val rc = RefCountKeyValueDB.forVersion(db, 1)
//      rc.del(ByteVector("doesnotexit".getBytes)).unsafeRunSync()
//      db.size.unsafeRunSync() shouldBe 0
//    }
//
//    "allow to rollback operations" in new Fixture {
//      val rc = RefCountKeyValueDB.forVersion(db, 1)
//      val inserted =
//        (1 to 4).map(i => ByteVector.fromValidHex(s"dead$i") -> ByteVector.fromValidHex(s"beef${i}")).toList
//      inserted.foreach(t => rc.put(t._1, t._2).unsafeRunSync())
...
//    }
//
//    "allow rollbacks after pruning" in new Fixture {
//      val rc = RefCountKeyValueDB.forVersion(db, 1)
//      val inserted =
//        (1 to 4).map(i => ByteVector.fromValidHex(s"dead$i") -> ByteVector.fromValidHex(s"beef${i}")).toList
//      inserted.foreach(t => rc.put(t._1, t._2).unsafeRunSync())
